...
function revSort(collection, key)
{
  var i, j;
  var count = collection.length;
  var parent, child;
...
var sortableProps = new Array("Date","CheckNumber","Description","TransType","Amount");
var transactions = new Array();

// We uppercase the tagName as a workaround for a bug
// that loses the original case of the tag.
// 13 May ekrock removed uppercasing but no effect
var acctset = document.getElementsByTagName("Transaction");
...
for (var i=0; i < acctset.length; i++) {
  transactions[i] = acctset[i];
}

collectInfo(transactions, sortableProps);
