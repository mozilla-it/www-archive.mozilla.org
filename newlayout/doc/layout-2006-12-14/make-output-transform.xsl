...
<xsl:template match="/html:html/html:body">
	<xsl:copy>
		...
		<xsl:apply-templates select="$slidediv/@*" />
		<xsl:text>&#x000A;</xsl:text>
		<html:h1>
			<xsl:variable name="header" select="$slidediv/*[position()=1 and self::html:h1]" />
			<html:span id="nav">
				<xsl:if test="$hasprev">
		...
		<xsl:apply-templates select="$slidediv/node()" />
		<xsl:text>&#x000A;</xsl:text>
	</xsl:copy>
</xsl:template>

<!-- Remove the headers whose contents we process above. -->
